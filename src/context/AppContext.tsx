...
export interface AppContextType {
  ...
  categories: Category[];
  theme: 'light' | 'dark';
  analyticsHistory: Record<number, number[]>; // year → monthlyExpenses
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [wallets, setWallets] = useState<Wallet[]>([{ id: 'personal', name: 'My Wallet', type: 'personal', balance: 0 }]);
  ...
  useEffect(() => localStorage.setItem('analyticsHistory', JSON.stringify(analyticsHistory)), [analyticsHistory]);
  ...
  const addWallet = (wallet: Omit<Wallet, 'id' | 'balance'>) => {
    setWallets(prev => [...prev, { ...wallet, id: Date.now().toString(), balance: 0 }]);
  };

  const addSavingsGoal = (goal: Omit<SavingsGoal, 'id'>): SavingsGoal => {
    const newGoal: SavingsGoal = { ...goal, id: Date.now().toString(), currentAmount: 0, members: goal.members || [] };
  ...
  const contributeToSaving = (goalId: string, amount: number, user: string) => {
    setSavingsGoals(prev =>
      prev.map(goal => {
        ...
        if (memberIndex >= 0) members[memberIndex].contribution = (members[memberIndex].contribution || 0) + amount;
        else members.push({ name: user, contribution: amount });

        const totalContribution = members.reduce((sum, m) => sum + (m.contribution || 0), 0);

        return { ...goal, members, currentAmount: Math.min(totalContribution, goal.targetAmount) };
      })
    );

...
const Analytics: React.FC = () => {
  const { transactions, categories, activeWallet, analyticsHistory, saveAnalytics } = useApp();
  ...
  useEffect(() => {
    const walletTransactions = transactions.filter(
      t => t.walletId === activeWallet && new Date(t.date).getFullYear() === currentYear
    );
    ...
    const isDifferent = !savedData || savedData.some((v,i) => v !== monthlyTotals[i]);
    if (isDifferent) {
  ...
  // Category breakdown
  const walletExpenses = transactions.filter(t => t.walletId === activeWallet && t.type === 'expense');

  const categoryData = categories
    .map(cat => {
      const total = walletExpenses.filter(t => t.category === cat.id).reduce((sum,t) => sum+t.amount,0);
      return { ...cat, total };
    })
    .filter(c => c.total > 0)
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-primary/5 to-secondary/5 pb-20">
      ...
      <div className="bg-gradient-primary p-6 rounded-b-[2rem] shadow-card">
        <div className="flex items-center justify-between">
          ...
          <Button variant="outline" size="sm" className="flex items-center gap-1 text-white hover:bg-white/20" onClick={() => navigate('/analytics-history')}>
            ...
            {monthlyData.map((amount,index) => (
              <div key={index}>
                <div className="flex items-center justify-between text-sm mb-1">
                  <span className="text-muted-foreground">{monthNames[index]}</span>
                  <span className="font-semibold">${amount.toFixed(2)}</span>
                </div>
                <div className="h-3 bg-muted rounded-full overflow-hidden">
                  <div className="h-full bg-gradient-primary transition-all duration-500" style={{width:`${(amount/maxExpense)*100}%`}}/>
                </div>
              </div>
            ))}
          </div>
            ...
            {categoryData.length > 0 ? categoryData.map(category => {
              ...
              return (
                <div key={category.id}>
                  ...
                  <div className="h-3 bg-muted rounded-full overflow-hidden">
                    <div className="h-full transition-all duration-500" style={{width:`${percentage}%`, backgroundColor: category.color}}/>
                  </div>
                </div>
              );
            }) : (
              <div className="text-center py-8">
                ...
                <p className="text-sm text-muted-foreground mt-1">Start adding expenses to see analytics</p>
              </div>
            )}
          </div>
        </Card>

    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-primary/5 to-secondary/5 pb-20">
      ...
      <div className="bg-gradient-primary p-6 rounded-b-[2rem] shadow-card">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-bold text-white" style={{fontFamily:'Fredoka'}}>Analytics History</h1>
        </div>
      ...
      <div className="px-6 mt-6 space-y-6">
        {filteredYears.length === 0 && (
          <div className="text-center py-8">
            <p className="text-muted-foreground">No analytics history available</p>
          </div>
        )}

        {filteredYears.map(year => {
          const monthlyData: number[] = analyticsHistory[year] || Array(12).fill(0);
          ...
          return (
            <Card key={year} className="p-6 shadow-card border-none">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-lg font-semibold">{year} - 12 Month Trend</h2>
              </div>
